...
`C03` identifies an honest unimplemented dependency.

| Requirement | Evidence/test | Status |
|---|---|---|
| immediate lineup component contract | `C04-contract.md`; legal-assignment tests | contract now; production adapter C04 |
| bye/absence component and week/slot/starter | `v6DraftIntegration.adversarial.test.ts`; `v6Explanation.contract.test.ts` | executable now |
| contingent role component/evidence/probability | same two suites | executable contract/C01 evidence now |
| breakout option component/upside basis | explanation contract | contract now; production adapter C04 |
| waiver replacement/churn component | accepted aggregate-only negative contract; skipped candidate adapter | C02 aggregate interface accepted; candidate evidence still blocked |
| redundancy cost component | explanation contract | contract now; value blocked C03 |
| evidence state measured/interpolated/unsupported/fallback | explanation contract | executable contract; producer blocked C03 |
...
| no hard cap from soft bench shape | skipped trace contract | blocked C03 |
...
| canonical/browser parity and drift failure | parity specification and skipped trace test | blocked C03 |
| no frontend Monte Carlo/simulation | static import/source guard | executable now |
